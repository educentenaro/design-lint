...
const root = await mkdtemp(join(tmpdir(), "design-lint-package-"));
...
try {
  ...
  const installed = process.platform === "win32" ? join(prefix, "node_modules", "design-lint") : join(prefix, "lib", "node_modules", "design-lint");
  ...
  assert.equal(pkg.bin["design-lint"], "./dist/cli.js");
  ...
  await writeFile(join(project, "design-lint.config.json"), JSON.stringify({ format: "json" }));
  ...
  const invoke = () => process.platform === "win32"
    ? run(process.env.ComSpec || "cmd.exe", ["/d", "/c", "design-lint scan"], { cwd: project, env })
    : run("design-lint", ["scan"], { cwd: project, env });
  const failed = invoke();
  ...
  console.log("Package verified: npm pack, isolated global install, external React project, failing and passing scans without Bun.");
} finally {
